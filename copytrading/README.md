...
```go
import (
    copytrading "github.com/openxapi/binance-go/copytrading"
)
```
...
```go
    conf := copytrading.NewConfiguration()
    client := copytrading.NewAPIClient(conf)
...
```go
    ...
    auth := copytrading.NewAuth(apiKey)    
...
For using other server than the one defined on index 0 set context value `copytrading.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), copytrading.ContextServerIndex, 1)
```
