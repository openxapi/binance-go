/*
Binance Convert API

OpenAPI specification for Binance exchange - Convert API

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package convert

import (
	"encoding/json"
)

// checks if the ConvertGetConvertTradeFlowV1RespListInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ConvertGetConvertTradeFlowV1RespListInner{}

// ConvertGetConvertTradeFlowV1RespListInner struct for ConvertGetConvertTradeFlowV1RespListInner
type ConvertGetConvertTradeFlowV1RespListInner struct {
	CreateTime *int64 `json:"createTime,omitempty"`
	FromAmount *string `json:"fromAmount,omitempty"`
	FromAsset *string `json:"fromAsset,omitempty"`
	InverseRatio *string `json:"inverseRatio,omitempty"`
	OrderId *int64 `json:"orderId,omitempty"`
	OrderStatus *string `json:"orderStatus,omitempty"`
	QuoteId *string `json:"quoteId,omitempty"`
	Ratio *string `json:"ratio,omitempty"`
	ToAmount *string `json:"toAmount,omitempty"`
	ToAsset *string `json:"toAsset,omitempty"`
}

// NewConvertGetConvertTradeFlowV1RespListInner instantiates a new ConvertGetConvertTradeFlowV1RespListInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConvertGetConvertTradeFlowV1RespListInner() *ConvertGetConvertTradeFlowV1RespListInner {
	this := ConvertGetConvertTradeFlowV1RespListInner{}
	return &this
}

// NewConvertGetConvertTradeFlowV1RespListInnerWithDefaults instantiates a new ConvertGetConvertTradeFlowV1RespListInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConvertGetConvertTradeFlowV1RespListInnerWithDefaults() *ConvertGetConvertTradeFlowV1RespListInner {
	this := ConvertGetConvertTradeFlowV1RespListInner{}
	return &this
}

// GetCreateTime returns the CreateTime field value if set, zero value otherwise.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetCreateTime() int64 {
	if o == nil || IsNil(o.CreateTime) {
		var ret int64
		return ret
	}
	return *o.CreateTime
}

// GetCreateTimeOk returns a tuple with the CreateTime field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetCreateTimeOk() (*int64, bool) {
	if o == nil || IsNil(o.CreateTime) {
		return nil, false
	}
	return o.CreateTime, true
}

// HasCreateTime returns a boolean if a field has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) HasCreateTime() bool {
	if o != nil && !IsNil(o.CreateTime) {
		return true
	}

	return false
}

// SetCreateTime gets a reference to the given int64 and assigns it to the CreateTime field.
func (o *ConvertGetConvertTradeFlowV1RespListInner) SetCreateTime(v int64) {
	o.CreateTime = &v
}

// GetFromAmount returns the FromAmount field value if set, zero value otherwise.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetFromAmount() string {
	if o == nil || IsNil(o.FromAmount) {
		var ret string
		return ret
	}
	return *o.FromAmount
}

// GetFromAmountOk returns a tuple with the FromAmount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetFromAmountOk() (*string, bool) {
	if o == nil || IsNil(o.FromAmount) {
		return nil, false
	}
	return o.FromAmount, true
}

// HasFromAmount returns a boolean if a field has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) HasFromAmount() bool {
	if o != nil && !IsNil(o.FromAmount) {
		return true
	}

	return false
}

// SetFromAmount gets a reference to the given string and assigns it to the FromAmount field.
func (o *ConvertGetConvertTradeFlowV1RespListInner) SetFromAmount(v string) {
	o.FromAmount = &v
}

// GetFromAsset returns the FromAsset field value if set, zero value otherwise.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetFromAsset() string {
	if o == nil || IsNil(o.FromAsset) {
		var ret string
		return ret
	}
	return *o.FromAsset
}

// GetFromAssetOk returns a tuple with the FromAsset field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetFromAssetOk() (*string, bool) {
	if o == nil || IsNil(o.FromAsset) {
		return nil, false
	}
	return o.FromAsset, true
}

// HasFromAsset returns a boolean if a field has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) HasFromAsset() bool {
	if o != nil && !IsNil(o.FromAsset) {
		return true
	}

	return false
}

// SetFromAsset gets a reference to the given string and assigns it to the FromAsset field.
func (o *ConvertGetConvertTradeFlowV1RespListInner) SetFromAsset(v string) {
	o.FromAsset = &v
}

// GetInverseRatio returns the InverseRatio field value if set, zero value otherwise.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetInverseRatio() string {
	if o == nil || IsNil(o.InverseRatio) {
		var ret string
		return ret
	}
	return *o.InverseRatio
}

// GetInverseRatioOk returns a tuple with the InverseRatio field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetInverseRatioOk() (*string, bool) {
	if o == nil || IsNil(o.InverseRatio) {
		return nil, false
	}
	return o.InverseRatio, true
}

// HasInverseRatio returns a boolean if a field has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) HasInverseRatio() bool {
	if o != nil && !IsNil(o.InverseRatio) {
		return true
	}

	return false
}

// SetInverseRatio gets a reference to the given string and assigns it to the InverseRatio field.
func (o *ConvertGetConvertTradeFlowV1RespListInner) SetInverseRatio(v string) {
	o.InverseRatio = &v
}

// GetOrderId returns the OrderId field value if set, zero value otherwise.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetOrderId() int64 {
	if o == nil || IsNil(o.OrderId) {
		var ret int64
		return ret
	}
	return *o.OrderId
}

// GetOrderIdOk returns a tuple with the OrderId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetOrderIdOk() (*int64, bool) {
	if o == nil || IsNil(o.OrderId) {
		return nil, false
	}
	return o.OrderId, true
}

// HasOrderId returns a boolean if a field has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) HasOrderId() bool {
	if o != nil && !IsNil(o.OrderId) {
		return true
	}

	return false
}

// SetOrderId gets a reference to the given int64 and assigns it to the OrderId field.
func (o *ConvertGetConvertTradeFlowV1RespListInner) SetOrderId(v int64) {
	o.OrderId = &v
}

// GetOrderStatus returns the OrderStatus field value if set, zero value otherwise.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetOrderStatus() string {
	if o == nil || IsNil(o.OrderStatus) {
		var ret string
		return ret
	}
	return *o.OrderStatus
}

// GetOrderStatusOk returns a tuple with the OrderStatus field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetOrderStatusOk() (*string, bool) {
	if o == nil || IsNil(o.OrderStatus) {
		return nil, false
	}
	return o.OrderStatus, true
}

// HasOrderStatus returns a boolean if a field has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) HasOrderStatus() bool {
	if o != nil && !IsNil(o.OrderStatus) {
		return true
	}

	return false
}

// SetOrderStatus gets a reference to the given string and assigns it to the OrderStatus field.
func (o *ConvertGetConvertTradeFlowV1RespListInner) SetOrderStatus(v string) {
	o.OrderStatus = &v
}

// GetQuoteId returns the QuoteId field value if set, zero value otherwise.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetQuoteId() string {
	if o == nil || IsNil(o.QuoteId) {
		var ret string
		return ret
	}
	return *o.QuoteId
}

// GetQuoteIdOk returns a tuple with the QuoteId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetQuoteIdOk() (*string, bool) {
	if o == nil || IsNil(o.QuoteId) {
		return nil, false
	}
	return o.QuoteId, true
}

// HasQuoteId returns a boolean if a field has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) HasQuoteId() bool {
	if o != nil && !IsNil(o.QuoteId) {
		return true
	}

	return false
}

// SetQuoteId gets a reference to the given string and assigns it to the QuoteId field.
func (o *ConvertGetConvertTradeFlowV1RespListInner) SetQuoteId(v string) {
	o.QuoteId = &v
}

// GetRatio returns the Ratio field value if set, zero value otherwise.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetRatio() string {
	if o == nil || IsNil(o.Ratio) {
		var ret string
		return ret
	}
	return *o.Ratio
}

// GetRatioOk returns a tuple with the Ratio field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetRatioOk() (*string, bool) {
	if o == nil || IsNil(o.Ratio) {
		return nil, false
	}
	return o.Ratio, true
}

// HasRatio returns a boolean if a field has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) HasRatio() bool {
	if o != nil && !IsNil(o.Ratio) {
		return true
	}

	return false
}

// SetRatio gets a reference to the given string and assigns it to the Ratio field.
func (o *ConvertGetConvertTradeFlowV1RespListInner) SetRatio(v string) {
	o.Ratio = &v
}

// GetToAmount returns the ToAmount field value if set, zero value otherwise.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetToAmount() string {
	if o == nil || IsNil(o.ToAmount) {
		var ret string
		return ret
	}
	return *o.ToAmount
}

// GetToAmountOk returns a tuple with the ToAmount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetToAmountOk() (*string, bool) {
	if o == nil || IsNil(o.ToAmount) {
		return nil, false
	}
	return o.ToAmount, true
}

// HasToAmount returns a boolean if a field has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) HasToAmount() bool {
	if o != nil && !IsNil(o.ToAmount) {
		return true
	}

	return false
}

// SetToAmount gets a reference to the given string and assigns it to the ToAmount field.
func (o *ConvertGetConvertTradeFlowV1RespListInner) SetToAmount(v string) {
	o.ToAmount = &v
}

// GetToAsset returns the ToAsset field value if set, zero value otherwise.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetToAsset() string {
	if o == nil || IsNil(o.ToAsset) {
		var ret string
		return ret
	}
	return *o.ToAsset
}

// GetToAssetOk returns a tuple with the ToAsset field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) GetToAssetOk() (*string, bool) {
	if o == nil || IsNil(o.ToAsset) {
		return nil, false
	}
	return o.ToAsset, true
}

// HasToAsset returns a boolean if a field has been set.
func (o *ConvertGetConvertTradeFlowV1RespListInner) HasToAsset() bool {
	if o != nil && !IsNil(o.ToAsset) {
		return true
	}

	return false
}

// SetToAsset gets a reference to the given string and assigns it to the ToAsset field.
func (o *ConvertGetConvertTradeFlowV1RespListInner) SetToAsset(v string) {
	o.ToAsset = &v
}

func (o ConvertGetConvertTradeFlowV1RespListInner) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ConvertGetConvertTradeFlowV1RespListInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.CreateTime) {
		toSerialize["createTime"] = o.CreateTime
	}
	if !IsNil(o.FromAmount) {
		toSerialize["fromAmount"] = o.FromAmount
	}
	if !IsNil(o.FromAsset) {
		toSerialize["fromAsset"] = o.FromAsset
	}
	if !IsNil(o.InverseRatio) {
		toSerialize["inverseRatio"] = o.InverseRatio
	}
	if !IsNil(o.OrderId) {
		toSerialize["orderId"] = o.OrderId
	}
	if !IsNil(o.OrderStatus) {
		toSerialize["orderStatus"] = o.OrderStatus
	}
	if !IsNil(o.QuoteId) {
		toSerialize["quoteId"] = o.QuoteId
	}
	if !IsNil(o.Ratio) {
		toSerialize["ratio"] = o.Ratio
	}
	if !IsNil(o.ToAmount) {
		toSerialize["toAmount"] = o.ToAmount
	}
	if !IsNil(o.ToAsset) {
		toSerialize["toAsset"] = o.ToAsset
	}
	return toSerialize, nil
}

type NullableConvertGetConvertTradeFlowV1RespListInner struct {
	value *ConvertGetConvertTradeFlowV1RespListInner
	isSet bool
}

func (v NullableConvertGetConvertTradeFlowV1RespListInner) Get() *ConvertGetConvertTradeFlowV1RespListInner {
	return v.value
}

func (v *NullableConvertGetConvertTradeFlowV1RespListInner) Set(val *ConvertGetConvertTradeFlowV1RespListInner) {
	v.value = val
	v.isSet = true
}

func (v NullableConvertGetConvertTradeFlowV1RespListInner) IsSet() bool {
	return v.isSet
}

func (v *NullableConvertGetConvertTradeFlowV1RespListInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConvertGetConvertTradeFlowV1RespListInner(val *ConvertGetConvertTradeFlowV1RespListInner) *NullableConvertGetConvertTradeFlowV1RespListInner {
	return &NullableConvertGetConvertTradeFlowV1RespListInner{value: val, isSet: true}
}

func (v NullableConvertGetConvertTradeFlowV1RespListInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConvertGetConvertTradeFlowV1RespListInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


