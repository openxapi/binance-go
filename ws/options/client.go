...
func NewServerManager() *ServerManager {
	...
	sm.servers["mainnet1"] = &ServerInfo{
		Name:        "mainnet1",
		URL:         "wss://nbstream.binance.com/eoptions/ws/{listenKey}",
...
func (c *Client) HandleRiskLevelChangeEvent(handler func(*models.RiskLevelChangeEvent) error) {
	c.eventHandler.RegisterHandler("riskLevelChangeEvent", func(data interface{}) error {
		...
		if jsonData, ok := data.([]byte); ok {
			// Direct JSON data parsing
			if err := json.Unmarshal(jsonData, &event); err != nil {
				return fmt.Errorf("failed to parse Risk Level Change Event event: %w", err)
			}
		} else if mapData, ok := data.(map[string]interface{}); ok {
			// Map data - check if this is the nested event object or the full message
			var eventDataToUnmarshal interface{}
			...
			jsonBytes, err := json.Marshal(eventDataToUnmarshal)
			if err != nil {
				return fmt.Errorf("failed to marshal Risk Level Change Event event data: %w", err)
			}
			if err := json.Unmarshal(jsonBytes, &event); err != nil {
				return fmt.Errorf("failed to parse Risk Level Change Event event: %w", err)
			}
		} else {
			return fmt.Errorf("unexpected data type for Risk Level Change Event event: %T", data)
		}
