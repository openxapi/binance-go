...
Module: github.com/openxapi/binance-go/ws/pmarginpro-streams
Version: 0.1.0
...
import (
  "context"
  "log"
  ws "github.com/openxapi/binance-go/ws/pmarginpro-streams"
  wsmodels "github.com/openxapi/binance-go/ws/pmarginpro-streams/models"
)
...
func main() {
  ...
  // _ = client.AddOrUpdateServer("alt", "wss://fstream.binance.com/pm-classic", "Alt Server", "Optional override")
...
```go
// Portfolio Margin Pro User Data Stream channel
ch := ws.NewUserDataStreamChannel(client)
// Register handler(s) before connect (recommended)
ch.HandleRiskLevelChangeEvent(func(ctx context.Context, ev *wsmodels.RiskLevelChangeEvent) error {
...
// Portfolio Margin Pro User Data Stream channel (requires listenKey)
...
```go
req := &wsmodels.RiskLevelChangeEvent{
