...
func NewClientWithOptions(opts *ClientOptions) *Client {
  ...
  _ = c.AddServer("mainnet1", "wss://ws-api.binance.com/ws-api/v3", "Binance Mainnet Server", "WebSocket server for binance exchange spot API (mainnet environment)")
  _ = c.AddServer("mainnet2", "wss://ws-api.binance.com:9443/ws-api/v3", "Binance Mainnet Server", "WebSocket server for binance exchange spot API (mainnet environment)")
  _ = c.AddServer("testnet1", "wss://ws-api.testnet.binance.vision/ws-api/v3", "Binance Testnet Server", "WebSocket server for binance exchange spot API (testnet environment)")
  _ = c.AddServer("testnet2", "wss://ws-api.testnet.binance.vision:9443/ws-api/v3", "Binance Testnet Server", "WebSocket server for binance exchange spot API (testnet environment)")
  return c
}
  ...
  if err := json.Unmarshal(data, &envelope); err == nil {
    ...
    if err := json.Unmarshal(payload, &typ); err == nil {
      ...
      if v, ok := typ["e"].(string); ok {
        ev = v
      } else if evobj, ok := typ["event"].(map[string]interface{}); ok {
        if vv, ok2 := evobj["e"].(string); ok2 {
          ev = vv
        }
      }
      if ev != "" {
      ...
      if err2 := json.Unmarshal(payload, &arr); err2 == nil && len(arr) > 0 {
        ...
        if err3 := json.Unmarshal(arr[0], &first); err3 == nil {
          ...
          if v, ok := first["e"].(string); ok {
            ev = v
          } else if evobj, ok := first["event"].(map[string]interface{}); ok {
            if vv, ok2 := evobj["e"].(string); ok2 {
              ev = vv
            }
          }
