/*
Binance Spot API

OpenAPI specification for Binance exchange - Spot API

API version: 0.3.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package spot

import (
	"encoding/json"
)

// checks if the GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp{}

// GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp struct for GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp
type GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp struct {
	EstRewardsInSOL *string `json:"estRewardsInSOL,omitempty"`
	Rows []GetSolStakingSolHistoryBnsolRewardsHistoryV1RespRowsInner `json:"rows,omitempty"`
	Total *int32 `json:"total,omitempty"`
}

// NewGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp instantiates a new GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp() *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp {
	this := GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp{}
	return &this
}

// NewGetSolStakingSolHistoryBnsolRewardsHistoryV1RespWithDefaults instantiates a new GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetSolStakingSolHistoryBnsolRewardsHistoryV1RespWithDefaults() *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp {
	this := GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp{}
	return &this
}

// GetEstRewardsInSOL returns the EstRewardsInSOL field value if set, zero value otherwise.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) GetEstRewardsInSOL() string {
	if o == nil || IsNil(o.EstRewardsInSOL) {
		var ret string
		return ret
	}
	return *o.EstRewardsInSOL
}

// GetEstRewardsInSOLOk returns a tuple with the EstRewardsInSOL field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) GetEstRewardsInSOLOk() (*string, bool) {
	if o == nil || IsNil(o.EstRewardsInSOL) {
		return nil, false
	}
	return o.EstRewardsInSOL, true
}

// HasEstRewardsInSOL returns a boolean if a field has been set.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) HasEstRewardsInSOL() bool {
	if o != nil && !IsNil(o.EstRewardsInSOL) {
		return true
	}

	return false
}

// SetEstRewardsInSOL gets a reference to the given string and assigns it to the EstRewardsInSOL field.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) SetEstRewardsInSOL(v string) {
	o.EstRewardsInSOL = &v
}

// GetRows returns the Rows field value if set, zero value otherwise.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) GetRows() []GetSolStakingSolHistoryBnsolRewardsHistoryV1RespRowsInner {
	if o == nil || IsNil(o.Rows) {
		var ret []GetSolStakingSolHistoryBnsolRewardsHistoryV1RespRowsInner
		return ret
	}
	return o.Rows
}

// GetRowsOk returns a tuple with the Rows field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) GetRowsOk() ([]GetSolStakingSolHistoryBnsolRewardsHistoryV1RespRowsInner, bool) {
	if o == nil || IsNil(o.Rows) {
		return nil, false
	}
	return o.Rows, true
}

// HasRows returns a boolean if a field has been set.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) HasRows() bool {
	if o != nil && !IsNil(o.Rows) {
		return true
	}

	return false
}

// SetRows gets a reference to the given []GetSolStakingSolHistoryBnsolRewardsHistoryV1RespRowsInner and assigns it to the Rows field.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) SetRows(v []GetSolStakingSolHistoryBnsolRewardsHistoryV1RespRowsInner) {
	o.Rows = v
}

// GetTotal returns the Total field value if set, zero value otherwise.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) GetTotal() int32 {
	if o == nil || IsNil(o.Total) {
		var ret int32
		return ret
	}
	return *o.Total
}

// GetTotalOk returns a tuple with the Total field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) GetTotalOk() (*int32, bool) {
	if o == nil || IsNil(o.Total) {
		return nil, false
	}
	return o.Total, true
}

// HasTotal returns a boolean if a field has been set.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) HasTotal() bool {
	if o != nil && !IsNil(o.Total) {
		return true
	}

	return false
}

// SetTotal gets a reference to the given int32 and assigns it to the Total field.
func (o *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) SetTotal(v int32) {
	o.Total = &v
}

func (o GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.EstRewardsInSOL) {
		toSerialize["estRewardsInSOL"] = o.EstRewardsInSOL
	}
	if !IsNil(o.Rows) {
		toSerialize["rows"] = o.Rows
	}
	if !IsNil(o.Total) {
		toSerialize["total"] = o.Total
	}
	return toSerialize, nil
}

type NullableGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp struct {
	value *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp
	isSet bool
}

func (v NullableGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) Get() *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp {
	return v.value
}

func (v *NullableGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) Set(val *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) {
	v.value = val
	v.isSet = true
}

func (v NullableGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) IsSet() bool {
	return v.isSet
}

func (v *NullableGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp(val *GetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) *NullableGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp {
	return &NullableGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp{value: val, isSet: true}
}

func (v NullableGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetSolStakingSolHistoryBnsolRewardsHistoryV1Resp) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


