/*
Binance Spot API

OpenAPI specification for Binance exchange - Spot API

API version: 0.3.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package spot

import (
	"encoding/json"
)

// checks if the GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate{}

// GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate struct for GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate
type GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate struct {
	Var05BTC *float32 `json:"0-5BTC,omitempty"`
	Var510BTC *float32 `json:"5-10BTC,omitempty"`
}

// NewGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate instantiates a new GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate() *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate {
	this := GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate{}
	return &this
}

// NewGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRateWithDefaults instantiates a new GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRateWithDefaults() *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate {
	this := GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate{}
	return &this
}

// GetVar05BTC returns the Var05BTC field value if set, zero value otherwise.
func (o *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) GetVar05BTC() float32 {
	if o == nil || IsNil(o.Var05BTC) {
		var ret float32
		return ret
	}
	return *o.Var05BTC
}

// GetVar05BTCOk returns a tuple with the Var05BTC field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) GetVar05BTCOk() (*float32, bool) {
	if o == nil || IsNil(o.Var05BTC) {
		return nil, false
	}
	return o.Var05BTC, true
}

// HasVar05BTC returns a boolean if a field has been set.
func (o *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) HasVar05BTC() bool {
	if o != nil && !IsNil(o.Var05BTC) {
		return true
	}

	return false
}

// SetVar05BTC gets a reference to the given float32 and assigns it to the Var05BTC field.
func (o *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) SetVar05BTC(v float32) {
	o.Var05BTC = &v
}

// GetVar510BTC returns the Var510BTC field value if set, zero value otherwise.
func (o *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) GetVar510BTC() float32 {
	if o == nil || IsNil(o.Var510BTC) {
		var ret float32
		return ret
	}
	return *o.Var510BTC
}

// GetVar510BTCOk returns a tuple with the Var510BTC field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) GetVar510BTCOk() (*float32, bool) {
	if o == nil || IsNil(o.Var510BTC) {
		return nil, false
	}
	return o.Var510BTC, true
}

// HasVar510BTC returns a boolean if a field has been set.
func (o *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) HasVar510BTC() bool {
	if o != nil && !IsNil(o.Var510BTC) {
		return true
	}

	return false
}

// SetVar510BTC gets a reference to the given float32 and assigns it to the Var510BTC field.
func (o *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) SetVar510BTC(v float32) {
	o.Var510BTC = &v
}

func (o GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Var05BTC) {
		toSerialize["0-5BTC"] = o.Var05BTC
	}
	if !IsNil(o.Var510BTC) {
		toSerialize["5-10BTC"] = o.Var510BTC
	}
	return toSerialize, nil
}

type NullableGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate struct {
	value *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate
	isSet bool
}

func (v NullableGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) Get() *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate {
	return v.value
}

func (v *NullableGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) Set(val *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) {
	v.value = val
	v.isSet = true
}

func (v NullableGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) IsSet() bool {
	return v.isSet
}

func (v *NullableGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate(val *GetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) *NullableGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate {
	return &NullableGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate{value: val, isSet: true}
}

func (v NullableGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetSimpleEarnFlexibleListV1RespRowsInnerTierAnnualPercentageRate) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


