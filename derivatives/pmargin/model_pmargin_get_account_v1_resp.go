/*
Binance Pmargin API

OpenAPI specification for Binance cryptocurrency exchange - Pmargin API

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package pmargin

import (
	"encoding/json"
)

// checks if the PmarginGetAccountV1Resp type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PmarginGetAccountV1Resp{}

// PmarginGetAccountV1Resp struct for PmarginGetAccountV1Resp
type PmarginGetAccountV1Resp struct {
	AccountEquity *string `json:"accountEquity,omitempty"`
	AccountInitialMargin *string `json:"accountInitialMargin,omitempty"`
	AccountMaintMargin *string `json:"accountMaintMargin,omitempty"`
	AccountStatus *string `json:"accountStatus,omitempty"`
	ActualEquity *string `json:"actualEquity,omitempty"`
	TotalAvailableBalance *string `json:"totalAvailableBalance,omitempty"`
	TotalMarginOpenLoss *string `json:"totalMarginOpenLoss,omitempty"`
	UniMMR *string `json:"uniMMR,omitempty"`
	UpdateTime *int64 `json:"updateTime,omitempty"`
	VirtualMaxWithdrawAmount *string `json:"virtualMaxWithdrawAmount,omitempty"`
}

// NewPmarginGetAccountV1Resp instantiates a new PmarginGetAccountV1Resp object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPmarginGetAccountV1Resp() *PmarginGetAccountV1Resp {
	this := PmarginGetAccountV1Resp{}
	return &this
}

// NewPmarginGetAccountV1RespWithDefaults instantiates a new PmarginGetAccountV1Resp object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPmarginGetAccountV1RespWithDefaults() *PmarginGetAccountV1Resp {
	this := PmarginGetAccountV1Resp{}
	return &this
}

// GetAccountEquity returns the AccountEquity field value if set, zero value otherwise.
func (o *PmarginGetAccountV1Resp) GetAccountEquity() string {
	if o == nil || IsNil(o.AccountEquity) {
		var ret string
		return ret
	}
	return *o.AccountEquity
}

// GetAccountEquityOk returns a tuple with the AccountEquity field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PmarginGetAccountV1Resp) GetAccountEquityOk() (*string, bool) {
	if o == nil || IsNil(o.AccountEquity) {
		return nil, false
	}
	return o.AccountEquity, true
}

// HasAccountEquity returns a boolean if a field has been set.
func (o *PmarginGetAccountV1Resp) HasAccountEquity() bool {
	if o != nil && !IsNil(o.AccountEquity) {
		return true
	}

	return false
}

// SetAccountEquity gets a reference to the given string and assigns it to the AccountEquity field.
func (o *PmarginGetAccountV1Resp) SetAccountEquity(v string) {
	o.AccountEquity = &v
}

// GetAccountInitialMargin returns the AccountInitialMargin field value if set, zero value otherwise.
func (o *PmarginGetAccountV1Resp) GetAccountInitialMargin() string {
	if o == nil || IsNil(o.AccountInitialMargin) {
		var ret string
		return ret
	}
	return *o.AccountInitialMargin
}

// GetAccountInitialMarginOk returns a tuple with the AccountInitialMargin field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PmarginGetAccountV1Resp) GetAccountInitialMarginOk() (*string, bool) {
	if o == nil || IsNil(o.AccountInitialMargin) {
		return nil, false
	}
	return o.AccountInitialMargin, true
}

// HasAccountInitialMargin returns a boolean if a field has been set.
func (o *PmarginGetAccountV1Resp) HasAccountInitialMargin() bool {
	if o != nil && !IsNil(o.AccountInitialMargin) {
		return true
	}

	return false
}

// SetAccountInitialMargin gets a reference to the given string and assigns it to the AccountInitialMargin field.
func (o *PmarginGetAccountV1Resp) SetAccountInitialMargin(v string) {
	o.AccountInitialMargin = &v
}

// GetAccountMaintMargin returns the AccountMaintMargin field value if set, zero value otherwise.
func (o *PmarginGetAccountV1Resp) GetAccountMaintMargin() string {
	if o == nil || IsNil(o.AccountMaintMargin) {
		var ret string
		return ret
	}
	return *o.AccountMaintMargin
}

// GetAccountMaintMarginOk returns a tuple with the AccountMaintMargin field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PmarginGetAccountV1Resp) GetAccountMaintMarginOk() (*string, bool) {
	if o == nil || IsNil(o.AccountMaintMargin) {
		return nil, false
	}
	return o.AccountMaintMargin, true
}

// HasAccountMaintMargin returns a boolean if a field has been set.
func (o *PmarginGetAccountV1Resp) HasAccountMaintMargin() bool {
	if o != nil && !IsNil(o.AccountMaintMargin) {
		return true
	}

	return false
}

// SetAccountMaintMargin gets a reference to the given string and assigns it to the AccountMaintMargin field.
func (o *PmarginGetAccountV1Resp) SetAccountMaintMargin(v string) {
	o.AccountMaintMargin = &v
}

// GetAccountStatus returns the AccountStatus field value if set, zero value otherwise.
func (o *PmarginGetAccountV1Resp) GetAccountStatus() string {
	if o == nil || IsNil(o.AccountStatus) {
		var ret string
		return ret
	}
	return *o.AccountStatus
}

// GetAccountStatusOk returns a tuple with the AccountStatus field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PmarginGetAccountV1Resp) GetAccountStatusOk() (*string, bool) {
	if o == nil || IsNil(o.AccountStatus) {
		return nil, false
	}
	return o.AccountStatus, true
}

// HasAccountStatus returns a boolean if a field has been set.
func (o *PmarginGetAccountV1Resp) HasAccountStatus() bool {
	if o != nil && !IsNil(o.AccountStatus) {
		return true
	}

	return false
}

// SetAccountStatus gets a reference to the given string and assigns it to the AccountStatus field.
func (o *PmarginGetAccountV1Resp) SetAccountStatus(v string) {
	o.AccountStatus = &v
}

// GetActualEquity returns the ActualEquity field value if set, zero value otherwise.
func (o *PmarginGetAccountV1Resp) GetActualEquity() string {
	if o == nil || IsNil(o.ActualEquity) {
		var ret string
		return ret
	}
	return *o.ActualEquity
}

// GetActualEquityOk returns a tuple with the ActualEquity field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PmarginGetAccountV1Resp) GetActualEquityOk() (*string, bool) {
	if o == nil || IsNil(o.ActualEquity) {
		return nil, false
	}
	return o.ActualEquity, true
}

// HasActualEquity returns a boolean if a field has been set.
func (o *PmarginGetAccountV1Resp) HasActualEquity() bool {
	if o != nil && !IsNil(o.ActualEquity) {
		return true
	}

	return false
}

// SetActualEquity gets a reference to the given string and assigns it to the ActualEquity field.
func (o *PmarginGetAccountV1Resp) SetActualEquity(v string) {
	o.ActualEquity = &v
}

// GetTotalAvailableBalance returns the TotalAvailableBalance field value if set, zero value otherwise.
func (o *PmarginGetAccountV1Resp) GetTotalAvailableBalance() string {
	if o == nil || IsNil(o.TotalAvailableBalance) {
		var ret string
		return ret
	}
	return *o.TotalAvailableBalance
}

// GetTotalAvailableBalanceOk returns a tuple with the TotalAvailableBalance field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PmarginGetAccountV1Resp) GetTotalAvailableBalanceOk() (*string, bool) {
	if o == nil || IsNil(o.TotalAvailableBalance) {
		return nil, false
	}
	return o.TotalAvailableBalance, true
}

// HasTotalAvailableBalance returns a boolean if a field has been set.
func (o *PmarginGetAccountV1Resp) HasTotalAvailableBalance() bool {
	if o != nil && !IsNil(o.TotalAvailableBalance) {
		return true
	}

	return false
}

// SetTotalAvailableBalance gets a reference to the given string and assigns it to the TotalAvailableBalance field.
func (o *PmarginGetAccountV1Resp) SetTotalAvailableBalance(v string) {
	o.TotalAvailableBalance = &v
}

// GetTotalMarginOpenLoss returns the TotalMarginOpenLoss field value if set, zero value otherwise.
func (o *PmarginGetAccountV1Resp) GetTotalMarginOpenLoss() string {
	if o == nil || IsNil(o.TotalMarginOpenLoss) {
		var ret string
		return ret
	}
	return *o.TotalMarginOpenLoss
}

// GetTotalMarginOpenLossOk returns a tuple with the TotalMarginOpenLoss field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PmarginGetAccountV1Resp) GetTotalMarginOpenLossOk() (*string, bool) {
	if o == nil || IsNil(o.TotalMarginOpenLoss) {
		return nil, false
	}
	return o.TotalMarginOpenLoss, true
}

// HasTotalMarginOpenLoss returns a boolean if a field has been set.
func (o *PmarginGetAccountV1Resp) HasTotalMarginOpenLoss() bool {
	if o != nil && !IsNil(o.TotalMarginOpenLoss) {
		return true
	}

	return false
}

// SetTotalMarginOpenLoss gets a reference to the given string and assigns it to the TotalMarginOpenLoss field.
func (o *PmarginGetAccountV1Resp) SetTotalMarginOpenLoss(v string) {
	o.TotalMarginOpenLoss = &v
}

// GetUniMMR returns the UniMMR field value if set, zero value otherwise.
func (o *PmarginGetAccountV1Resp) GetUniMMR() string {
	if o == nil || IsNil(o.UniMMR) {
		var ret string
		return ret
	}
	return *o.UniMMR
}

// GetUniMMROk returns a tuple with the UniMMR field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PmarginGetAccountV1Resp) GetUniMMROk() (*string, bool) {
	if o == nil || IsNil(o.UniMMR) {
		return nil, false
	}
	return o.UniMMR, true
}

// HasUniMMR returns a boolean if a field has been set.
func (o *PmarginGetAccountV1Resp) HasUniMMR() bool {
	if o != nil && !IsNil(o.UniMMR) {
		return true
	}

	return false
}

// SetUniMMR gets a reference to the given string and assigns it to the UniMMR field.
func (o *PmarginGetAccountV1Resp) SetUniMMR(v string) {
	o.UniMMR = &v
}

// GetUpdateTime returns the UpdateTime field value if set, zero value otherwise.
func (o *PmarginGetAccountV1Resp) GetUpdateTime() int64 {
	if o == nil || IsNil(o.UpdateTime) {
		var ret int64
		return ret
	}
	return *o.UpdateTime
}

// GetUpdateTimeOk returns a tuple with the UpdateTime field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PmarginGetAccountV1Resp) GetUpdateTimeOk() (*int64, bool) {
	if o == nil || IsNil(o.UpdateTime) {
		return nil, false
	}
	return o.UpdateTime, true
}

// HasUpdateTime returns a boolean if a field has been set.
func (o *PmarginGetAccountV1Resp) HasUpdateTime() bool {
	if o != nil && !IsNil(o.UpdateTime) {
		return true
	}

	return false
}

// SetUpdateTime gets a reference to the given int64 and assigns it to the UpdateTime field.
func (o *PmarginGetAccountV1Resp) SetUpdateTime(v int64) {
	o.UpdateTime = &v
}

// GetVirtualMaxWithdrawAmount returns the VirtualMaxWithdrawAmount field value if set, zero value otherwise.
func (o *PmarginGetAccountV1Resp) GetVirtualMaxWithdrawAmount() string {
	if o == nil || IsNil(o.VirtualMaxWithdrawAmount) {
		var ret string
		return ret
	}
	return *o.VirtualMaxWithdrawAmount
}

// GetVirtualMaxWithdrawAmountOk returns a tuple with the VirtualMaxWithdrawAmount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PmarginGetAccountV1Resp) GetVirtualMaxWithdrawAmountOk() (*string, bool) {
	if o == nil || IsNil(o.VirtualMaxWithdrawAmount) {
		return nil, false
	}
	return o.VirtualMaxWithdrawAmount, true
}

// HasVirtualMaxWithdrawAmount returns a boolean if a field has been set.
func (o *PmarginGetAccountV1Resp) HasVirtualMaxWithdrawAmount() bool {
	if o != nil && !IsNil(o.VirtualMaxWithdrawAmount) {
		return true
	}

	return false
}

// SetVirtualMaxWithdrawAmount gets a reference to the given string and assigns it to the VirtualMaxWithdrawAmount field.
func (o *PmarginGetAccountV1Resp) SetVirtualMaxWithdrawAmount(v string) {
	o.VirtualMaxWithdrawAmount = &v
}

func (o PmarginGetAccountV1Resp) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PmarginGetAccountV1Resp) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.AccountEquity) {
		toSerialize["accountEquity"] = o.AccountEquity
	}
	if !IsNil(o.AccountInitialMargin) {
		toSerialize["accountInitialMargin"] = o.AccountInitialMargin
	}
	if !IsNil(o.AccountMaintMargin) {
		toSerialize["accountMaintMargin"] = o.AccountMaintMargin
	}
	if !IsNil(o.AccountStatus) {
		toSerialize["accountStatus"] = o.AccountStatus
	}
	if !IsNil(o.ActualEquity) {
		toSerialize["actualEquity"] = o.ActualEquity
	}
	if !IsNil(o.TotalAvailableBalance) {
		toSerialize["totalAvailableBalance"] = o.TotalAvailableBalance
	}
	if !IsNil(o.TotalMarginOpenLoss) {
		toSerialize["totalMarginOpenLoss"] = o.TotalMarginOpenLoss
	}
	if !IsNil(o.UniMMR) {
		toSerialize["uniMMR"] = o.UniMMR
	}
	if !IsNil(o.UpdateTime) {
		toSerialize["updateTime"] = o.UpdateTime
	}
	if !IsNil(o.VirtualMaxWithdrawAmount) {
		toSerialize["virtualMaxWithdrawAmount"] = o.VirtualMaxWithdrawAmount
	}
	return toSerialize, nil
}

type NullablePmarginGetAccountV1Resp struct {
	value *PmarginGetAccountV1Resp
	isSet bool
}

func (v NullablePmarginGetAccountV1Resp) Get() *PmarginGetAccountV1Resp {
	return v.value
}

func (v *NullablePmarginGetAccountV1Resp) Set(val *PmarginGetAccountV1Resp) {
	v.value = val
	v.isSet = true
}

func (v NullablePmarginGetAccountV1Resp) IsSet() bool {
	return v.isSet
}

func (v *NullablePmarginGetAccountV1Resp) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePmarginGetAccountV1Resp(val *PmarginGetAccountV1Resp) *NullablePmarginGetAccountV1Resp {
	return &NullablePmarginGetAccountV1Resp{value: val, isSet: true}
}

func (v NullablePmarginGetAccountV1Resp) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePmarginGetAccountV1Resp) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


